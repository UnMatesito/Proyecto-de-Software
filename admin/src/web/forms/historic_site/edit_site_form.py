...
class EditSiteForm(CreateSiteForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if kwargs.get("site"):
            ...
            super(EditSiteForm, self).__init__(*args, name=site.name, 
            ...
            self.province.choices = [
                ...
                (province.id, province.name) for province in get_all_provinces() if province.id != site.city.province.id
            ]

            self.city.choices = [
                (site.city.id, site.city.name)
            ] + [  # Cargo las provincias en el select
                (city.id, city.name) for city in get_all_cities() if city.id != site.city.id and city.province.id == site.city.province.id
            ]

            self.conservation_state.choices = [
                (site.conservation_state.id, site.conservation_state.state)
            ] + [  # Cargo los estados en el select
                (state.id, state.state) for state in get_all_conservation_state() if state.id != site.conservation_state.id
            ] 

            self.category.choices = [
                (site.category.id, site.category.name)
            ] + [  # Cargo las categorias en el select
                (category.id, category.name) for category in get_all_categories() if category.id != site.category.id
            ]
