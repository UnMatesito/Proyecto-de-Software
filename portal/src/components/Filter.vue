<template>
  <form class="m-auto">
    <div class="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
      ...
      <div class="relative">
        <div class="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
          <svg class="w-4 h-4 text-gray-500" xmlns="http://www.w3.org/2000/svg" fill="none"
               viewBox="0 0 20 20"><path stroke="currentColor" stroke-linecap="round"
               stroke-linejoin="round" stroke-width="2"
               d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z"/></svg>
        </div>
        <input type="search" v-model="nameValue"
      ...
      <!-- Favoritos (disabled si no está autenticado) -->
      <div class="p-2 border rounded-md text-sm bg-gray-50 dark:bg-gray-700 dark:border-gray-600 relative"
           :class="!authStore.isAuthenticated ? 'opacity-50 cursor-not-allowed group' : ''">
        <input id="fav-check" v-model="favoriteValue" type="checkbox"
               :disabled="!authStore.isAuthenticated"
               class="text-blue-600 bg-gray-100 border-gray-300 rounded-sm">
        <label for="fav-check" class="ms-2">Favoritos</label>
      ...
      <!-- Botón Filtrar -->
      <div class="col-start-2 row-start-3 md:col-start-4 md:row-start-1 flex gap-1">
        <router-link
          :to="{
            query: {
      ...
      <!-- Ciudad -->
      <select id="default" v-model="cityValue"
      ...
      <div class="relative">
        ...
        <input type="search" v-model="descrpitionValue"
      ...
      <!-- Orden -->
      <select id="order" v-model="orderByValue"
...
const fetchCities = async (provinceName) => {
  ...
  if (province) {
    ...
    if (!rout.query.city) cityValue.value = 'Ciudad'
...
// Reaccionar a cambios en props.tags
watch(
  () => props.tags,
  async () => {
    await nextTick()
